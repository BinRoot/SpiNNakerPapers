...
One very important property of the brain that has received little attention by Computer Scientists, which is also believed to be quintessential in mankind's ability to adapt to and solve new and ever-more difficult problems, is neural plasticity.
It would be erroneous to say that no degree of plasticity has been implemented in any type of SNN, but given that we do not yet fully understand the mechanisms governing neural plasticity in our own heads, it suffices to say that there is yet much work to be done in this field of computational neuroscience in which artificial neural networks dynamically alter their own topology.
...
Rank order, under the same constraints, is capable of achieving $log_2(n!)$ bits of information. 
...
In short, Rank Order (RO) coding is assumed to be a better model as it can process more information in a shorter amount of time with fewer redundant neurons. 
...
We will later describe a new learning method referred to as entropic synaptogenetic plasticity (ESP), governed by biologically inspired formulas taken from (Levy), and influence of Information Theory from Shannon.

\subsection{Types of Spiking Neural Networks}
[TODO: Matt]

\begin{itemize}
\item[(a)] {\bf SNN} (default, outdated) - static
\item[(b)] {\bf eSNN} (evolves -alters synaptic weights- between a learning and recall phase)
\item[(c)] {\bf deSNN} (evolves dynamically - there's no differentiation between learning and recall (learns continuously))
\end{itemize}
...
\item capable of running complex SNN simulations
\item scalable
\item new
\end{itemize}
...
For example, defining how the neurons are connected together greatly affects the effeciency and performance in learning [cite]. 
Perceptrons are often designed as bipartite graphs, where every neuron in the presynaptic layer is connected to every neuron in the postsynaptic layer.
This brute-force approach to constructing a network topology may achieve satisfactory results if the weights converge to the optimal values within an acceptible time.
...
Unlike most networks, we allow our's to form new connections between neurons while learning occurs. 
The creation of a new synapse depends on whether the postsynaptic neuron is at an optimal activation. 
...
Avidity is the measure of ablility for each neuron to participate in a new synaptic connection, defined as
...
The receptivity of new synaptic connections is inversly proportional to the running average of the neuron's activation.
...
Similar to most unsupervised weight modification rules, we use a deterministic process depending on the pre- and postsynaptic activities.
This type of weight adjustement is based off the Hebbian rule, where links between nodes that fire together strengthens.
...
Lastly, we allow our network to undergo synaptic removal, a stoachastic process where the probability of removal becomes non-zero if excitatory synapses fall bellow a specified threshold.
