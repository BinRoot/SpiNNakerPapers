...
\begin{itemize}
\item[(a)] {\bf Static} - The network topology is fixed, and can only change by manually adjusting the relatiionship between nodes. Most neural networks fall into this category because regardless of the number of nodes \(n\), only one fixed relationship forms between them:
...
\item[(b)] {\bf Semi-Dynamic} - The network is not static, and there is some freedom for nodes to rewire with other nodes in real time. The complexity grows polynomially. Each node has non-zero probablity to be rewired with a constant \(c\) number of other nodes. Given \(n\) such nodes, the number of possible topologies can be up to
...
Every input node broadcasts a data packet to six output nodes, but not all messages are registed by the output neurons. Each packet received by an output node is looked up in the output node's local list of incoming connections. If the address from the packet is not listed in the incoming connections list, it will be ignored, and no further computation will be done.  

Synaptic connectivity was broken when weights fell below some negative threshold. For some \(\delta > 0\), when an average running weight dropped below \(-\delta\), the link was considered disconnected.

New connections were established between nodes when an average rate of activity dropped below 25\%. In this case, the next data packet received from a muted neuron was unmuted.
